...
use crate::skills::types::{
    AppType, InstalledSkill, SkillApps, SkillRepo, SkillStorageLocation, SyncMethod, UnmanagedSkill,
};
use crate::skills::paths::{get_app_config_dir, get_home_dir, get_hermes_dir, get_mcode_dir, get_pi_agent_dir};
use crate::skills::db::Database;
use crate::skills::error::format_skill_error;
...
fn get_agents_skills_dir() -> Option<PathBuf> {
    let dir = get_home_dir().join(".agents").join("skills");
...
impl SkillService {
    ...
    pub fn get_ssot_dir() -> Result<PathBuf> {
        let location = crate::skills::settings::get_skill_storage_location();
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        match app {
            AppType::Claude => {
                if let Some(custom) = crate::skills::settings::get_claude_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::ClaudeDesktop | AppType::Mcode => {}
            AppType::Codex => {
                if let Some(custom) = crate::skills::settings::get_codex_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::Gemini => {
                if let Some(custom) = crate::skills::settings::get_gemini_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::GrokBuild => {
                if let Some(custom) = crate::skills::settings::get_grok_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::OpenCode => {
                if let Some(custom) = crate::skills::settings::get_opencode_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::OpenClaw => {
                if let Some(custom) = crate::skills::settings::get_openclaw_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::Hermes => {
                if let Some(custom) = crate::skills::settings::get_hermes_override_dir() {
                    return Ok(custom.join("skills"));
                }
            }
            AppType::Pi => {
                return Ok(get_pi_agent_dir()?.join("skills"));
            }
        }
        ...
        Ok(match app {
            AppType::Mcode => get_mcode_dir().join("skills"),
            ...
            AppType::Hermes => get_hermes_dir().join("skills"),
            AppType::Pi => get_pi_agent_dir()?.join("skills"),
        })
    }
    ...
    pub fn migrate_storage(
        ...
        let _state_guard = skill_state_write_guard();
        let current = crate::skills::settings::get_skill_storage_location();
        if current == target {
        ...
        // 3. 文件移动完成后才持久化设置
        crate::skills::settings::set_skill_storage_location(target)?;
    ...
    fn get_sync_method() -> SyncMethod {
        crate::skills::settings::get_skill_sync_method()
    }
    ...
    async fn download_and_extract(&self, url: &str, dest: &Path) -> Result<()> {
        let client = crate::skills::http::client();
